...
### Added
- **i18n locale set expanded to 37 languages** (v0.2.123, two rounds).
  - **Round 1**: 10 → 33 locales. Added Traditional Chinese variants `zh-HK` / `zh-TW`; renamed `zh` → `zh-CN` so the three Chinese variants sit as siblings. Legacy `'zh'` storage values are migrated in `initSettings()` and accepted via `LEGACY_ALIAS` in `i18n/index.ts` (handles bare `zh` tag from generic-Chinese browsers and pre-rename users). 23 new LTR languages cover Chrome Web Store Top 30, tiered by ROI: high (ko / it / nl / pl / tr / vi / id), mid (sv / da / fi / cs / el / hu / ro / th), long-tail (nb / uk / bg / hr / sk / ca).
  - **Round 2**: 33 → 37 locales. Added the 4 remaining Chrome Web Store Top 30 RTL languages: Hebrew (`he`), Persian (`fa`), Urdu (`ur`), Pashto (`ps`). `RTL_LOCALES` extended from `{ar}` to `{ar, he, fa, ur, ps}`; `<html dir="rtl">` is now set automatically by `setLocale()` for any of the 5 RTL locales. Full bidi layout polish (column mirroring, button icons) is tracked separately.
  - Background SW `SETTINGS_MENU_TITLES` updated to 37 entries (also keeps `'zh'` legacy key alongside `'zhCN'`).
  - 25 new catalog files in `src/lib/i18n/catalog/`: bg / ca / cs / da / el / fa / fi / he / hr / hu / id / it / ko / nb / nl / pl / ps / ro / sk / sv / th / tr / uk / ur / vi / zh-CN / zh-HK / zh-TW (the rename `zh.ts` → `zh-CN.ts` is a delete + add).
- **2-letter ISO 3166-1 country code prefix in the language `<select>`** (v1.1.13). Each option now reads `CODE  selfName (englishName)` (e.g. `HK  中文（香港） (Chinese (Hong Kong))`, `TW  ...`, `CN  ...`). The code lives in a new `LOCALE_REGION_CODES: Record<LocaleCode, string>` map keyed by `LocaleCode` so TS forces it to stay in sync with `LOCALE_FLAGS`.
- **Auto-expand first level of folders after layout import** (v1.1.16). The settings panel's "Include layout" import now writes `open.col.<id> = true` to `chrome.storage.local` for every top-level non-special folder id, and seeds a one-shot `importExpandOverride: Set<string>` in `folder.ts` consumed by `checkOpenState` so the expansion works even when `rememberOpen` is off. Sub-folders inside the expanded top-level folders are NOT marked — only the first level opens.

### Changed
- **Language dropdown uses ISO 3166-1 country code only — flag emoji dropped** (v1.1.14). Earlier v1.1.12 had added Unicode Regional Indicator flag emoji (`🇭🇰` / `🇹🇼` / `🇨🇳`); v1.1.14 drops the flag emoji and keeps only the ISO code. Reasons: (1) flag-emoji rendering is platform-dependent — some Chrome/macOS combinations render `🇭🇰` / `🇹🇼` ambiguously, defeating the point of the prefix; (2) the distinction between the three Chinese variants is politically sensitive in some regions, and shipping a flag emoji invites that whole debate. A 2-letter ISO code is unambiguous, renders identically everywhere, and stays out of it.
- **RTL link/folder text alignment** (v1.1.10). When the document is `<html dir="rtl">`, the text inside `.link-text` is explicitly right-aligned so it hugs the icon (which sits at the right edge of the row in the natural RTL flex order). Plus a small `margin-inline-start: 0.5em` on `.link-text` (v1.1.11) for breathing room between the text and the icon — the logical property works in both LTR and RTL.

### Fixed
- **Import layout false positive: deleted ids dropped alongside valid ones** (v1.1.15). The native `chrome.bookmarks.get(ids, callback)` batch API has a fatal behaviour: when **any one** of the requested ids doesn't exist (deleted, never-existed, etc.), the entire batch call fails — `results = undefined` and `chrome.runtime.lastError` is set to `"Can't find bookmark for id."` — rather than emitting a partial result. The wrapper's `lastError` branch then mapped every position to `undefined`, so a layout JSON with 4 valid ids + 1 deleted id (e.g. `1753` in the test case) had all 5 ids reported in the "已跳过 5 个项目" alert and dropped — even the 4 that were perfectly valid. Fix: rewrite `getBookmarks` to `Promise.all(ids.map(id => getBookmark(id)))` — each id now succeeds or fails independently. `getBookmark` already handles `lastError` and resolves to `undefined` for "id not found" (v1.1.7); `Promise.all` preserves input order.

### Bundle
- newtab gzipped: 26.91KB → ~30KB (was 29.95KB at v1.1.16). Still well under the 80KB budget. The 25 new locale catalogs add ~25KB uncompressed (~10KB gzipped) but Vite tree-shakes anything the user never selects.

### Internal
- 25 new catalog files in `src/lib/i18n/catalog/`. `src/lib/i18n/types.ts` and `src/lib/i18n/index.ts` updated to support the larger locale set. `src/background.ts` updated to ship the 37-language context-menu title table. `src/features/settings/apply.ts` and `src/lib/storage/settings.ts` updated to handle the `'zh'` → `'zh-CN'` storage migration. `pnpm-lock.yaml` updated. `CLAUDE.md` §0.1 + §6.5 updated to reflect the expanded locale set.

## [v0.2.123] - 2026-07-06

### Added
- **Locale set expanded from 10 → 33 → 37** (two rounds).
  - **Round 1 (10 → 33)**: 23 new LTR locales covering Chrome Web Store Top 30, plus Traditional Chinese variants `zh-HK` / `zh-TW`. The `zh` → `zh-CN` rename makes the three Chinese variants siblings in the `SUPPORTED_LOCALES` array. `LocaleCode` type is now a 33-element union, so the `Record<LocaleCode, X>` compile-time exhaustiveness checks (used by `CATALOG` and `LOCALE_FLAGS` / `LOCALE_REGION_CODES`) now cover all 33 supported locales. Old `'zh'` storage values are migrated at `initSettings()` and accepted at lookup time via `LEGACY_ALIAS` in `i18n/index.ts` (handles bare `zh` tags from generic-Chinese browsers and pre-rename users).
  - **Round 2 (33 → 37)**: 4 new RTL locales — `he` / `fa` / `ur` / `ps` — the remaining Chrome Web Store Top 30 RTL languages. `RTL_LOCALES` extended from `{ar}` to `{ar, he, fa, ur, ps}`; `<html dir="rtl">` is now set automatically by `setLocale()` for any of the 5 RTL locales.
- **Background SW `SETTINGS_MENU_TITLES` table updated to 37 entries** ([`src/background.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/background.ts)). Keeps the `'zh'` legacy key alongside `'zhCN'` so users with old `Settings.language` values don't see a "Chrome 扩展" / "設定" mismatch in the browser action context menu. `chrome.storage.onChanged` listener already re-creates the context menu on language change — no behaviour change there, just the table grew.

### Changed
- **`'zh'` → `'zh-CN'` rename**. Simplifies locale matching (the 3 Chinese variants now sit as 3 sibling entries in `SUPPORTED_LOCALES` rather than 1 entry that needs region subtag handling). Storage migration + `LEGACY_ALIAS` keep the transition transparent for users with old settings.
- **`SUPPORTED_LOCALES` JSDoc updated** ([`src/lib/i18n/types.ts:13-31`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/types.ts#L13-L31)) with the tier-by-ROI breakdown of the 23 new LTR entries.
- **`i18n/index.ts` header comment updated** ([`src/lib/i18n/index.ts:12-19`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/index.ts#L12-L19)) with the bundle-budget math (10 → 33 → 37 locale growth → ~16KB → ~55KB → ~60KB gzipped).

### Bundle
- newtab gzipped: 26.91KB → 29.87KB. Still well under the 80KB budget. The 25 new locale catalogs add ~25KB uncompressed (~10KB gzipped) but Vite tree-shakes anything the user never selects. Bundle code-splitting is intentionally NOT done — it would force an async flash on the first locale switch.

## [1.1.16] - 2026-07-06

### Added
- **Auto-expand first level of folders after layout import**. When the settings panel's "Include layout" import is applied, the top-level non-special folder ids (the ids that appear in `nextColumns` after the v1.1.15 dropped-id filter) are now also expanded on the first render. Two mechanisms working together: (1) the import flow writes `open.col.<id> = true` to `chrome.storage.local` for each, so the expansion persists across page reloads when the user has `rememberOpen` on (the default); (2) it seeds a new one-shot override set in [`src/features/bookmarks/folder.ts:45-66`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts#L45-L66) via the new exported `markFoldersExpandedOnce(ids)`, consumed (deleted) the first time [`checkOpenState`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts#L301-L321) sees the id, so the expansion also works when `rememberOpen` is OFF (in which case the storage write is a no-op but the override ensures the first-render expand still happens). The read-once consumption is the key behaviour: after the first render the override is gone, so a subsequent page refresh / `renderColumns` re-render goes through the normal `rememberOpen` + storage path — meaning a user who collapses an imported folder after seeing it expanded will see it stay collapsed on the next refresh. Sub-folders inside the expanded top-level folders are NOT marked (no override, no storage write) — they stay at whatever the storage / default state is, so only the first level opens, matching the user's "展开 1369，但是不展开里面的子目录" request. The id set is built from `nextColumns` (post-filter) and deduped with `new Set` to avoid redundant writes when an id sits in more than one column. Implementation: [`settings-panel.ts:2616-2637`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L2616-L2637) (caller) + [`folder.ts:57-66`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts#L57-L66) (`importExpandOverride` + `markFoldersExpandedOnce`) + [`folder.ts:301-321`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts#L301-L321) (`checkOpenState` consume-first). No new MessageKey (no user-facing string). Bundle impact: ~120 bytes uncompressed in the newtab chunk (one module-level Set + one exported function + the override check); newtab bundle 29.91 → ~29.93 KB gzipped.

## [1.1.15] - 2026-07-06

### Fixed
- **Import layout false positive: deleted ids dropped alongside valid ones**. The import-layout validator in [`settings-panel.ts:2514`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L2514) calls `getBookmarks(nonSpecialIds)` to batch-validate every non-special id in the imported `layout.columns` against the current Chrome bookmark tree. The wrapper at [`src/lib/chrome/bookmarks.ts:39-73`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/chrome/bookmarks.ts#L39-L73) was calling the native `chrome.bookmarks.get(ids, callback)` batch API. That API has a fatal behaviour: when **any one** of the requested ids doesn't exist (deleted, never-existed, etc.), the entire batch call fails — `results = undefined` and `chrome.runtime.lastError` is set to `"Can't find bookmark for id."` — rather than emitting a partial result with the missing id absent. The wrapper's `lastError` branch then mapped every position in the result array to `undefined`, so a layout JSON with 4 valid ids + 1 deleted id (e.g. `1753` in the test case) had all 5 ids reported in the "已跳过 5 个项目" alert and dropped from the imported layout — even the 4 that were perfectly valid. Repro: export a settings JSON whose `layout.columns` contains 4 real folder ids + 1 deleted folder id, then re-import. Fix: rewrite `getBookmarks` at [`src/lib/chrome/bookmarks.ts:57-62`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/chrome/bookmarks.ts#L57-L62) to `Promise.all(ids.map(id => getBookmark(id)))` — each id now succeeds or fails independently. The single-id `getBookmark` wrapper already reads `chrome.runtime.lastError` and resolves to `undefined` for the "id not found" case (v1.1.7), so per-id semantics are unchanged. `Promise.all` preserves the input-array order so the validator's index-based lookup stays correct. Bundle impact: negligible (the wrapper is in `lib/chrome/`, not the newtab chunk); no behaviour change for other call sites since this function has only the one caller (the import-layout validator).

## [1.1.14] - 2026-07-06

### Changed
- **Language dropdown no longer shows flag emoji — only the 2-letter ISO code**. The v1.1.12 feature added a Unicode Regional Indicator flag emoji prefix to each option in the settings panel's "Language" `<select>`; v1.1.13 then added a 2-letter ISO code as a textual fallback. v1.1.14 drops the flag emoji and keeps only the ISO code. Two reasons: (1) flag-emoji rendering is platform-dependent — some Chrome/macOS combinations render 🇭🇰 / 🇹🇼 ambiguously, defeating the whole point of having the prefix; (2) the visual distinction between the three Chinese variants (`zh-CN` / `zh-HK` / `zh-TW`) is politically sensitive in some regions, and shipping a flag emoji invites that whole debate. A 2-letter ISO code is unambiguous, renders identically on every platform, and stays out of it. Code change summary: removed the `LOCALE_FLAGS: Record<LocaleCode, string>` map and its re-export from [`src/lib/i18n/types.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/types.ts) and [`src/lib/i18n/index.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/index.ts); removed the `flag` import and its `parts.push(flag)` line in [`createLanguageSelect` at settings-panel.ts:1566-1594](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L1566-L1594). The `'auto'` option stays code-less. Final layout per option is now `HK  中文（香港） (Chinese (Hong Kong))`, `TW  中文（台湾） (Chinese (Taiwan))`, `CN  中文 (Chinese, Simplified)`, etc. — all 37 entries follow the same `CODE  selfName (englishName)` shape. The `LOCALE_REGION_CODES` map itself is unchanged; the visual elements are simpler. Bundle impact: a small reduction (37 Regional Indicator Symbol pairs removed); newtab bundle drops from 29.92 KB to ~29.85 KB gzipped.

## [1.1.13] - 2026-07-06

### Added
- **2-letter ISO country code as a textual fallback in the language dropdown**. The v1.1.12 flag-emoji prefix doesn't render reliably on every platform — some Chrome emoji fonts (and some macOS / Windows / Linux combinations) render 🇭🇰 / 🇹🇼 ambiguously or skip the Regional Indicator Symbol pairs entirely, leaving the user unable to tell `zh-HK` / `zh-TW` / `zh-CN` apart. Each option in the language `<select>` is now prefixed with `flag  CODE  selfName (englishName)` (e.g. `🇭🇰  HK  中文（香港） (Chinese (Hong Kong))`, `🇹🇼  TW  ...`, `🇨🇳  CN  ...`). The 2-letter ISO 3166-1 alpha-2 code is a textual fallback that always displays correctly regardless of emoji font, so the three Chinese variants are visually distinct on every platform. The 37 codes live in a new `LOCALE_REGION_CODES: Record<LocaleCode, string>` map at [`src/lib/i18n/types.ts:174-212`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/types.ts#L174-L212) — keyed by `LocaleCode` with the same lockstep-with-`LOCALE_FLAGS` constraint, so TS forces both maps to be updated when a 38th locale is added. Selection rules mirror `LOCALE_FLAGS`: region-subtag locales use that region's code (`zh-HK` → `HK`, `zh-TW` → `TW`); regionless locales use the "origin" code by Chrome's own convention (`en` → `US`, `pt` → `PT`, `ar` → `SA`); `ca` (Catalan) → `ES` (no Catalonia ISO code); `ps` (Pashto) → `AF`. The `'auto'` option is intentionally code-less — it means "follow the browser", not a specific region. The prefix assembly is at [`createLanguageSelect` in settings-panel.ts:1580-1598](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L1580-L1598). Visual layout is `flag  code  label` with a double-space separator so the pieces stay grouped at a glance. The 4 parts are joined into one string, so the whole prefix lives on the visual start side (left in LTR, right in RTL) without any CSS mirroring — same Unicode Bidi behavior as the v1.1.12 flag prefix. Bundle impact: the 37 2-letter strings are ~150 bytes uncompressed; the newtab bundle stays well under the 80KB gzipped budget.

## [1.1.12] - 2026-07-06

### Added
- **Region flags in the language dropdown**. Each of the 37 locale options in the settings panel's "Language" `<select>` is now prefixed with its region flag (e.g. `🇺🇸 English (English)`, `🇨🇳 中文 (Chinese, Simplified)`, `🇭🇰 中文 (Chinese, Hong Kong)`, `🇹🇼 中文 (Chinese, Traditional)`). The flags live in a new `LOCALE_FLAGS: Record<LocaleCode, string>` map at [`src/lib/i18n/types.ts:117-155`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/types.ts#L117-L155), keyed by `LocaleCode` so TypeScript catches any missing flag entry at the moment a 38th locale is added. Each flag is a Unicode Regional Indicator Symbol pair (e.g. `🇺🇸` = U+1F1FA + U+1F1F8) — zero-asset, no SVG, no extra bundle weight. Selection rules per locale: region subtag locales (`zh-CN` / `zh-HK` / `zh-TW`) use that region's flag; origin-country convention for regionless ones (`en` → 🇺🇸 matching Chrome, `pt` → 🇵🇹 not 🇧🇷, `ar` → 🇸🇦 for Modern Standard Arabic); `ca` (Catalan) falls back to 🇪🇸 since there's no country code for Catalonia; `ps` (Pashto) → 🇦🇫 (Afghanistan, where Pashto is a national language). The `'auto'` option is intentionally flag-less. The flag is prepended to the `<option>` textContent at [`createLanguageSelect` in settings-panel.ts:1570-1577](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L1570-L1577); the browser lays out `<option>` text per the document `direction`, so the flag lands on the visual start side (left in LTR, right in RTL) without any CSS mirroring.

## [1.1.11] - 2026-07-06

### Fixed
- **Small breathing room between link/folder title text and the icon**. With the v1.1.10 RTL text-align fix, the title text now hugs the right edge of `.link-text` and ends up flush against the icon's left edge — no visible gap. The `padding` on `#main a` is the row edge (not the inter-element gap), so it doesn't help here. Fix: added `margin-inline-start: 0.5em` to [`.link-text`](file:///Users/lingsmbp/Documents/aiwork/newtab01/styles/newtab.css#L215-L227). Uses the logical `margin-inline-start` property so the gap lands on the icon side in both directions: LTR (icon on the left → gap on the left of the text) and RTL (icon on the right → gap on the right of the text). Plain physical `margin-left` would only fix one direction. The same `.link-text` class is shared by bookmark links ([link.ts](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/link.ts)) and folder headers ([folder.ts](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts)), so both get the gap. The folder action icons (open-all / group / split) sit on the opposite side of the text and are not affected.

## [1.1.10] - 2026-07-06

### Fixed
- **RTL: link and folder title text now hugs the icon (right edge of the row)**. In RTL locales (Arabic / Hebrew / Persian / Urdu / Pashto), the natural flex order places the icon on the right and the `.link-text` span to its left (`flex: 1` takes the remaining width). The text inside `.link-text` should be right-aligned within the span so the characters sit at the right edge — touching the icon's left edge. The default `text-align: start` should map to right in RTL (the parent `<a>` inherits `direction: rtl` from `<html dir="rtl">`), but in practice the `flex: 1` + `text-align: start` combo landed the text in the middle of the span rather than at the right edge. Fix: explicit `[dir="rtl"] .link-text { text-align: right; }` at [globals.css:177-193](file:///Users/lingsmbp/Documents/aiwork/newtab01/styles/globals.css#L177-L193). Applies to both bookmark links ([link.ts:64-67](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/link.ts#L64-L67)) and folder headers ([folder.ts:75-78](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/folder.ts#L75-L78)) — both use the same `.link-text` span for the title. The flex order itself (icon on the right, actions on the left for folders) is unchanged from the natural RTL mirror.

## [1.1.9] - 2026-07-06

### Added
- **Locale set expanded 33 → 37 — full Chrome Web Store top-30 RTL coverage**. The four remaining RTL languages (Hebrew `he`, Persian `fa`, Urdu `ur`, Pashto `ps`) are now first-class locales in the settings panel's language dropdown, joining Arabic `ar` (the first RTL locale the extension shipped with, in v0.2.119). New catalogs: [`src/lib/i18n/catalog/he.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/catalog/he.ts) (Modern Hebrew, selfName "עברית") / [`fa.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/catalog/fa.ts) (Persian, selfName "فارسی", typographic «...» / "..." quote marks) / [`ur.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/catalog/ur.ts) (Urdu, selfName "اردو", Naskh-style vocabulary) / [`ps.ts`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/catalog/ps.ts) (Pashto, selfName "پښتو", literary register). All four carry the full ~250 MessageKey set so `tsc` (`as const satisfies LocaleMessages`) enforces parity with `en.ts` — adding a 38th locale without translating it would fail the build, not silently fall through at runtime. `<html dir="rtl">` is set automatically by `applyDocumentAttributes()` because the new entries are listed in [`RTL_LOCALES`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/i18n/types.ts#L85); the existing `[dir="rtl"]` rules in `styles/globals.css` (search-result URL direction, search input alignment) cover the four new locales without code changes.
- **Background SW settings-menu title table expanded to 37 entries** in [`src/background.ts:86-90`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/background.ts#L86-L90) with localized "Open Settings" strings for the four new RTL locales (`he` / `fa` / `ur` / `ps`).

### Fixed
- **Settings panel header (title + close aria-label) now refreshes on language switch**. When the user opened the settings panel and then changed the active language, the panel's top title text and the close button's `aria-label` stayed frozen at the value of the language that was active when the panel first opened — both were created exactly once in [`renderPanelHeader()`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts) and never re-rendered. The body (nav + content) was already re-painted by `refreshSettingsPanelLocale` (added in v0.2.123) but the header was missed. Fix: extended [`refreshSettingsPanelLocale`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L802-L812) to also update `.sp-title` `textContent` to `t('settings.title')` and `.sp-close` `aria-label` to `t('settings.close')`. Matches the existing pattern in `applyLocaleToDom` for the open-settings panel — no new MessageKey.
- **Settings panel nav no longer shows a horizontal scrollbar for long tab labels**. [`.sp-nav-item`](file:///Users/lingsmbp/Documents/aiwork/newtab01/styles/newtab.css#L1066-L1087) had `white-space: nowrap` (inherited from a generic button reset), so labels longer than the 140px-wide nav column ("Темы оформления", "Configurações", "Personnalisation", etc.) overflowed horizontally and produced a horizontal scrollbar on the nav aside. Fix: changed to `white-space: normal; word-break: break-word;` (i.e. `word-wrap: break-word`). `break-word` (not `break-all`) preserves natural break points — short labels stay on a single line, long ones wrap mid-word only when there's no other option. The nav aside is `flex-shrink: 0` and `overflow-y: auto`; after the fix the only scrollbar is the (intended) vertical one. No new MessageKey, no behavior change.

### Notes
- **Bundle size**: newtab gzipped 29.78KB → 29.87KB (+90 bytes for 4 new locales × ~22 bytes each compressed). Still well under the 80KB budget in CLAUDE.md §7.
- **RTL UI scope**: the four new locales are functionally complete in the i18n catalog and the document-direction set, but their **bidi layout is only partially polished**. `<html dir="rtl">` is set, search-result URLs and the search input direction are handled, but full bidirectional work (column visual order, button icon mirroring, etc.) is tracked separately per the user's "稍后我们再来调整 rtl 的 ui" decision. The user can switch to any of the 5 RTL locales in the dropdown and the strings will be correct; if the layout looks "off" in some places, that's the deferred-work bucket.
- **AI-translated catalog quality**: 4 new catalogs (he / fa / ur / ps) are AI-generated per the v0.2.117 policy in `docs/i18n.md`. Right-to-left script fonts can render some of the typographic conventions inconsistently; manual proofread suggested for `he` and `fa` first (highest user-facing traffic from Chrome Web Store top-30 RTL).

## [1.1.8] - 2026-07-06

### Added
- **Locale set expanded 10 → 33**. The settings panel's language dropdown now covers the Chrome Web Store top-30 LTR languages (Arabic / Hebrew / Persian / Urdu / Pashto excluded as per request; Arabic was already supported and stays) plus two explicit Traditional Chinese variants for Hong Kong (`zh-HK`) and Taiwan (`zh-TW`). New catalogs: `ko` (한국어) / `it` (Italiano) / `nl` (Nederlands) / `pl` (Polski) / `tr` (Türkçe) / `vi` (Tiếng Việt) / `id` (Bahasa Indonesia) / `sv` (Svenska) / `da` (Dansk) / `fi` (Suomi) / `cs` (Čeština) / `el` (Ελληνικά) / `hu` (magyar) / `ro` (română) / `th` (ไทย) / `nb` (norsk bokmål) / `uk` (українська) / `bg` (български) / `hr` (hrvatski) / `sk` (slovenčina) / `ca` (català) + `zh-HK` + `zh-TW`. Each catalog carries the full ~250 MessageKey set so tsc (`as const satisfies LocaleMessages`) enforces parity — adding a 34th locale without translating it would fail the build, not silently fall through at runtime.
- **Background SW settings-menu title table expanded to 33 entries** in [`src/background.ts:43-86`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/background.ts#L43-L86). The legacy bare `'zh'` key is kept alongside the new `'zhCN'` key (both resolve to "打开设置") so pre-rename users still get the right title until their stored value is migrated by `initSettings()` on first launch.
- **`zh-HK` and `zh-TW` chrome._locales directories** ([`_locales/zh_HK/messages.json`](file:///Users/lingsmbp/Documents/aiwork/newtab01/_locales/zh_HK/messages.json) + [`_locales/zh_TW/messages.json`](file:///Users/lingsmbp/Documents/aiwork/newtab01/_locales/zh_TW/messages.json)) so the Chrome Web Store dashboard unlocks the per-locale store-listing editor for the two Traditional variants. Translations follow the same 132-character manifest description cap as the existing 10 chrome._locales entries.

### Changed
- **Simplified Chinese was renamed `zh` → `zh-CN`** so the two Traditional variants can live as siblings in `SUPPORTED_LOCALES`. Three layers of backward-compat make this transparent to existing users: ① `initSettings()` in [`src/lib/storage/settings.ts:414-418`](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/storage/settings.ts#L414-L418) rewrites any stored `'zh'` to `'zh-CN'` on first launch and persists the change back to `chrome.storage.sync`; ② `i18n/index.ts → LEGACY_ALIAS` maps the bare `'zh'` tag to `'zh-CN'` in `resolveLocale` for users who never persisted (brand-new install) or for browsers / OSes that report the generic `'zh'` tag (rare, but exists); ③ the SW settings-menu table keeps the `'zh'` key with the same Chinese string. The 23 new `MessageKey` translations are all AI-generated (per v0.2.117 policy in `docs/i18n.md`) — `hi` / `ar` / `ru` / `th` / `el` / `uk` / `bg` are the lowest-confidence ones and should be human-reviewed before claiming full localization.

### Notes
- **Bundle size**: newtab gzipped 26.91KB → 29.78KB (+2.87KB for 23 new locales × ~125 bytes each compressed). Still well under the 80KB budget in CLAUDE.md §7.
- **Ascii-apostrophe fix in `ca.ts` and `uk.ts`**: Catalan contractions (d'interès → d’interès) and Ukrainian genitive apostrophes (з'єднання → з’єднання) use U+2019 (right single quotation mark) instead of ASCII `'` so the JS string-literal delimiter doesn't have to be escaped in the catalog files. This is the same typographic convention French / Italian / English use; the rendered text is visually identical.
...
### Fixed
- **Drag-back-to-origin no longer pollutes the undo stack**. A drop that ends up at the same state as before (e.g. user drags a folder within its own multi-folder column to the slot it already occupied — `addRow` removes A from the source and re-inserts at the same position, so `columns` comes out structurally identical) used to push a snapshot onto the undo stack. The undo badge ticked up but pressing undo did nothing — the recorded "step" was a no-op. Fix: new `isSnapshotEqual(snapshot, columns, movedOut)` helper in [history.ts:73-116](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/history.ts#L73-L116) compares both `columns` (outer length, per-column length, ids in order) and `movedOut` (keys, per-key id arrays in order). Wired into both push points in [drop-handler.ts:213-216, 284-289](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/drop-handler.ts#L213-L216) via a small `isLayoutUnchanged()` helper that snapshots `getColumns()` + `await getMovedOut()` and runs the diff. When the post-mutation state is identical to the captured pre-mutation snapshot, the snapshot is discarded and the function returns early. Order matters in both fields — `addRow` / `removeRow` rely on the user-perceived order being preserved through the snapshot. The two existing early-returns (self-drop on single-folder source column at [drop-handler.ts:166-175](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/drop-handler.ts#L166-L175) and `lockColumns` refusal at [drop-handler.ts:228-237](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/drop-handler.ts#L228-L237)) are unaffected — they return before the snapshot is even captured, so no diff check is needed there.

## [1.1.5] - 2026-07-05

### Fixed
- **Link drag is now disabled; phantom undo entries from link drops are gone**. `<a href>` elements have `draggable=true` in the browser's default UA stylesheet, so grabbing a bookmark link fired a native link drag whose `dragstart` bubbled up to the parent column div. The column's `enableDragColumn` handler then called `setDragIds` with EVERY folder id in the column ([drag-column.ts:26-28](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/drag-column.ts#L26-L28)). On release the drop handler ran, found no actual change (the user dropped on the same column), and pushed a snapshot onto the undo stack — the user saw "undo shows a step but nothing reverts". Fix: set `a.draggable = false` in `renderLink` at [link.ts:52](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/link.ts#L52) so neither the native link drag nor the bubble-to-column side effect can fire. The Apps special folder is the one link that should still be draggable, and it already re-enables `draggable=true` via `enableDragFolder` ([link.ts:168-170](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/link.ts#L168-L170)) which also calls `event.stopPropagation()` in its dragstart handler — Apps behaves like a folder, not a regular link.

## [1.1.4] - 2026-07-05

### Fixed
- **Import settings: layout block with stale bookmark ids no longer breaks the board**. A settings JSON whose `layout` was exported from a different Chrome (or earlier in this Chrome's lifetime) may contain ids that no longer point to a folder — the user deleted the folder, repurposed the id as a regular link, or re-organised bookmarks so the id is now a descendant. Previously the import accepted all non-empty string ids and tried to render them; the resulting column contained a node that didn't render and the column was effectively empty (no context menu, no drop target). Now the import filter in [settings-panel.ts:2440-2516](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/newtab/settings-panel.ts#L2440-L2516) batch-validates every imported id against `chrome.bookmarks.get(ids)` in a single round-trip (new `getBookmarks(ids[])` wrapper in [lib/chrome/bookmarks.ts:21-59](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/lib/chrome/bookmarks.ts#L21-L59)) and drops any that resolve to a link or no longer exist. Special folder ids (`apps` / `top` / `recent` / `closed` / `devices` / `1` / `2`) are still always accepted — they are runtime stubs, not chrome bookmark ids. The dropped count is surfaced via `window.alert` (new `MessageKey` `settings.advanced.importLayoutDropped`, all 10 catalogs) so the user knows the layout was trimmed. The import itself still succeeds — silently dropping is the worst UX because the user can't tell whether the layout was applied.
- **Empty column is now actionable**. When the only folder in a column is deleted from Chrome bookmarks, the column div renders nothing and — more importantly — the column-level context menu is never attached, so the user can't right-click → remove the column or drag a new folder in. Previously `board.ts`'s `col.length === 0 → false` filter also hid the column entirely in the rare case the layout itself was empty. Fix in [board.ts:26-94](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/board.ts#L26-L94) + [column.ts:14-143](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/bookmarks/column.ts#L14-L143) + [drop-handler.ts:61-94,180-208](file:///Users/lingsmbp/Documents/aiwork/newtab01/src/features/drag-drop/drop-handler.ts#L61-L94): keep the column div in the DOM (the drop handler reads the column index off `.column` siblings, so this is required for drops to work), render a "Drop a folder here" placeholder as a `<a class="folder folder--empty">` element wrapped in `<ul><li>`, and attach the column-level context menu via a new `renderEmptyColumnPlaceholder()` helper. The placeholder reuses the regular `a.folder` rule (font-size: 1.8em, line-height: 1.4, flex + align-items: center) so the placeholder text is guaranteed to land at the exact same y position as the first folder in a regular column — no hard-coded margin/padding offset that would drift on font-size changes. The placeholder is reached from all three empty branches of `renderColumn` (rawIds empty / single-id showRoot-false resolves to null / multi-folder all resolve to null) so any combination of deleted-folder + hidden-special falls into the same UX. **Drop behavior**: the empty column is a drop TARGET, not a column-structure boundary. The drop handler's `onDragOver` checks `.column--empty` BEFORE the `.column` branch (the empty column has both classes) and shows a 4px solid border around the whole column instead of a left/right border, signalling "drop here fills the column". The drop handler's `captureAndDrop` then routes single-folder drops onto the empty column through `addRow(id, x, 0)` instead of the column-structure branch — the latter would have created a new column to the left/right of the empty slot, which is the wrong UX. CSS in [newtab.css:395-456](file:///Users/lingsmbp/Documents/aiwork/newtab01/styles/newtab.css#L395-L456) — `.column--empty` uses `display: inline-flex` (NOT `flex`; the other columns are `inline-block` and sit in a row by virtue of the parent's inline layout, so `flex` would break the empty column out of the row and stack it alone below the rest). `.folder--empty` is a placeholder-only modifier: muted color, no hover background, no pointer, no user-select. Dashed border for the slot. New `MessageKey` `column.empty`, all 10 catalogs. The width divisor is the count of actually-rendered columns (so the visible columns keep their equal-width 1/N distribution regardless of how many were filtered for hidden-special-only).

### Changed
- **`board.ts` now iterates the original `columns` array** (instead of a filtered copy) when computing column indices for `enableDragColumn` / `renderColumn`. Pre-existing off-by-one when a hidden-special column sat between two visible columns is now also fixed (the visible index no longer collides with a layout index that pointed at a hidden-special column).
...
### Added
- **「回退」按钮下方加一行浮动 hint 文字**。文案：「仅本次会话可回退，刷新后清空」/ "Only undoable in this session; clears on refresh"。新 MessageKey `undo.sessionHint` 已加进 10 个 catalog（en / zh / es / ar / hi / fr / pt / de / ja / ru），tsc 编译期校验 0 error。
- 解释 in-memory history stack 行为：拖拽/编辑布局后变更已写入 `chrome.storage.local`，刷新页面相当于把当前布局 apply 落地，刷新之后回退栈清空，之前的旧布局无法再 undo。

### Changed
- **回退按钮旁边加一行 hint 浮动文字**。按钮本身保持原始 box-model 不变（`padding: 7px 14px` + `font-size: 1.2rem` + `line-height: 1.4`，渲染高度 ~33px），不强行去匹配搜索框的高度。hint 用 `position: absolute`（`top: calc(100% + 4px)`）锚到 `.undo-wrap` 容器，浮在按钮下方 4px —— 因为脱离文档流，**不影响 topbar 的 flex 布局**，搜索框和按钮在 topbar 里依然干净对齐，hint 像一个浮动的 label 一样 hover 在按钮下。
- **DOM 结构**：回退按钮 + hint 包在一个 `.undo-wrap` div 里（`position: relative`，作为 hint 的定位锚点）。`margin-left: 8px` 从 `#undo_button` 搬到 `.undo-wrap`。wrap 跟按钮一起 hide（`display: none`）—— history stack 为空时连同小字一起消失，避免出现"空 hint 列"。
- 10 个 locale 同步加 `undo.sessionHint` 翻译（项目自有文案，AI 翻译生成；hi / ar / ru 优先人工校对）。

### Notes
- **设计迭代历史（未发布）**：feat/undo-button-polish 分支上的 4 轮迭代（按钮高度对齐搜索框 → 缩小 padding 让 wrap 对齐搜索框 → hint 内联进按钮 → 最终方案 hint 浮动在按钮下方）都保留在 git history 里作为索引，但 CHANGELOG 只记录最终发布的 v1.0.15 状态。如果想看每个 rev 的取舍，checkout `feat/undo-button-polish` 分支看那 5 个 commit。
- **副作用**：hint 浮在按钮下方会跟书签列的第一行重叠 —— 视觉上 hint 跟下面的 bookmark 距离更近了。
...
### Fixed
- **`vite.config.ts` `PUBLIC_DESCRIPTION`** —— dist/manifest.json 的 `description` 字段从 138 字符（英文）改为 42 字符（中文）。Chrome Web Store 校验上限 132 字符，旧版超 6 字符导致上架被拒。新文案：`是基于书签的新标签页，支持以 group 和 split view 形式打开标签目录`。

### Notes
- **manifest 字段变更**（按 CLAUDE.md §11.1 反例清单），**bump patch** v1.0.10 → v1.0.11。
- `pnpm package` 跑完后 `dist/manifest.json` 的 description 字段会写入新文案，可以直接上传 Chrome Web Store。
- 源 manifest.json 的 description 仍是 "Source manifest..." 警告文案（给 dev 看的，build 时由 `fixupDistManifest()` 覆盖成 PUBLIC_NAME / PUBLIC_DESCRIPTION）。
- 三处版本号同步：`manifest.json` / `package.json` / `src/lib/version.ts` 全部为 `1.0.11`。
- **未发 GitHub Release** —— v1.0.9 仍为 store 提交周期锚点。store 上架前手动 `gh release create v1.0.11` 即可。
